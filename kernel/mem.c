...
vm_page_free(void *pa)
{
  ...
  // in the table.
  // YOUR CODE HERE
  struct frame *A = (struct frame *)pa;
...
vm_page_insert(pagetable_t pagetable, uint64 va, uint64 pa, int perm)
{
    ...
    p = walk_pgtable(pagetable, va, 1);
    if(p == 0)
    {
      return -1;
    }

    *p = PA2PTE(pa) | perm | PTE_V;
    return 0;
